...
ns Unroll(const vector<vector<int>>& Matrix, const vector<int>& Vec, vector<long long>& Result)
{
    ...
    auto      lambda       = [&](int Row, int Col, vector<long long>& Result) {
        if (Col < n) Result[Col] += Matrix[Row][Col] * Vec[Row];
    };
    for (int Row = 0; Row < n; ++Row)
        for (int Col = 0; Col < n; Col += UnrollFactor)
            UnrollLoop<UnrollFactor>::Do(
                [&](int R, int C, vector<long long>& Res) { lambda(R, Col + C, Res); }, Row, Result);
    auto End = hrClk::now();
    return chrono::duration_cast<ns>(End - Start);
}
