...
void ThSIMD::SSE::PerformGaussianBlur(uint8_t* Output, const uint8_t* OriImg, int Width, int Height)
{
...
void ThSIMD::AVX::A256::PerformGaussianBlur(uint8_t* Output, const uint8_t* OriImg, int Width, int Height)
{
    std::vector<float>       Tmp(Width * Height, 0.0f);
    int                      ThreadsNum = std::thread::hardware_concurrency();
    std::vector<std::thread> Threads(ThreadsNum);
...
void ThSIMD::AVX::A512::PerformGaussianBlur(uint8_t* Output, const uint8_t* OriImg, int Width, int Height)
{
    int                      ThreadsNum = std::thread::hardware_concurrency();
    std::vector<std::thread> Threads(ThreadsNum);
    float*                   Tmp = (float*)_mm_malloc(Width * Height * sizeof(float), 64);
