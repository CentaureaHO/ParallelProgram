...
int main(int argc, char* argv[])
{
    ...
    for (const auto& Image : Images)
    {
        ...
        int paddedStartRow = max(0, startRow - 2);
        int paddedEndRow   = min(TmpHeight, endRow + 2);
        ...
        uint8_t* RecvBuffer = (uint8_t*)_mm_malloc(TmpWidth * TmpHeight * sizeof(uint8_t), 64);

        for (int step = 0; step < size; step++)
        {
            int partner = (rank - step + size) % size;
            MPI_Sendrecv(EdgeArray,
                TmpWidth * TmpHeight,
                MPI_UINT8_T,
                next,
                0,
            ...
            if (partner != rank)
            {
                ...
                {
                    EdgeArray[i] =
                        std::max(EdgeArray[i], RecvBuffer[i]);
                }
            }
        }
        ...
        if (rank == 0)
        {
            cv::Mat finalImage(TmpHeight, TmpWidth, CV_8UC1, EdgeArray);
            cv::resize(
                finalImage, finalImage, cv::Size(OriginalWidth, OriginalHeight));
            cv::imwrite(OutName, finalImage);
    ...
    if (rank == 0)
    {
        ofstream csvFile("../CannyRecord/runtime/MPI_Ring_" + to_string(size) + ".csv");
        csvFile << "Image,ProcessingTime(s)" << endl;
        for (size_t i = 0; i < Images.size(); ++i) { csvFile << Images[i] << "," << processingTimes[i] << endl; }
        csvFile.close();
    }
